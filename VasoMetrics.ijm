...
macro "VasoMetrics Action Tool - C059T3e16V" {
	...
	getDateAndTime(year, month, dayOfWeek, DOM, hour, minute, second, msec);
	if (!File.exists(settingsDir)) {
		File.close(File.open(settingsDir));
		File.append("LastUpdateDOM:" + DOM + "\n", settingsDir);	
	} else {
		settings = File.openAsString(settingsDir);
		settingsArray = split(settings, "\n");
		updateLine = settingsArray[0];
